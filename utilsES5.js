...
utils.skinnedInput = function(el) {
	...
	if(type == 'datetime-local' || type == 'select-one' || type == 'select-multiple') { //Datetime or Select:
		...
		var si=siChange.bind(el); el.addEventListener('change',si); el.forceUpdate=si; si();
	}
...
utils.bounds = function(n, min, max) {
	if(min==null) min=0; if(max==null) max=1;
...
utils.norm = utils.normalize = function(n, min, max) {
	if(min==null) min=0; if(max==null) max=1; var c = Math.abs(max-min);
