...
type ParentEntry = ParEntryType | ParEntrySchema;
...
const isDictOrArr = (d: any) => typeof d === 'object' && d !== null,
	isDict = (d: any) => isDictOrArr(d) && !Array.isArray(d),
	oobStr = (s: SizedEntry) => ` out-of-bounds (${s.min == null ? '*' : s.min}-${s.max == null ? '*' : s.max})`,
	runHook = <T>(t: T | Hook<T>, k: any, v: any, o: Options): T =>
		typeof t === 'function' ? (t as Hook<T>)(o._p, {k, v, r: o._r}) : t;

const dictFmt = (s: any) => {
	if(typeof s.c === 'string') s.c = {t: s.c};
	const dt = isDictOrArr(s.f) ? 2 : isDictOrArr(s.c) ? 1 : 0;
	if(!dt) throw 'Schema lacks format or childType';
	if(dt === 2 && s.c) throw 'Cannot require both format and childType';
	return dt === 2;
};

const tryAny = <T>(a: T | T[], fn: (v: T) => void) => {
	if(Array.isArray(a)) {
		const el: any[] = [];
...
function isNumArr(a: any[]) {
	for(const v of a) if(typeof v !== 'number') return;
	return 1;
}
...
function prettyJSON(val: any, _d = 0) {
	...
	if(t === 'number' || t === 'string' || t === 'boolean') return JSON.stringify(val);
		...
		for(k in val) {
			...
			s += (ln ? f ? ',\n' : '\n' : f ? ', ' : '') + '\t'.repeat(ln ? d1 : 0)
				+ JSON.stringify(k) + ':' + prettyJSON(val[k], d1);
			f = 1, ln = 0;
		}
		if(!_d) s += '\n';
		return s + '}';
